...
iptables -A OUTPUT -o $IN_DEV -p tcp --dport 80 -m state --state ESTABLISHED -j ACCEPT
...
# User management:
# allow
iptables -t mangle -I internet 1 -m mac --mac-source USER-MAC-ADDRESS-HERE -s USER-IP -j RETURN

# block
iptables -t mangle -D internet  -m mac --mac-source USER_MAC_ADDRESS -s USER-IP -j RETURN
